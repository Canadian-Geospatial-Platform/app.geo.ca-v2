        ...
        <ul class="pagination pagination-list2 text-center">
            {#if pgcnt > 1 }
                <li class={cgroup === 1 ? 'list-item first disabled' : 'list-item first'}>
                    <button
                        ...
                        on:click={cgroup > 1 ? () => dispatch('handleRelatedPage',{pnum:((cgroup - 1) * ppg)}) : undefined}
            ...
            <li class={current === 1 ? 'list-item previous disabled' : 'list-item previous'}>
                <button
                    ...
                    on:click={current > 1 ? () =>dispatch('handleRelatedPage', {pnum:(current - 1)}) : undefined}
            ...
            {#each pagenumbers as pn}
                <li class={pn !== current ? 'list-item' : 'list-item current disabled'}>
                    <button
                        ...
                        on:click={pn !== current ? () => dispatch('handleRelatedPage',{pnum:pn}) : undefined}
                    >
                        {pn}                        
                    </button>
                </li>
            ...
            <li class={current === pcnt ? 'list-item next disabled' : 'list-item next'}>
                <button
                    ...
                    on:click={current < pcnt ? () =>dispatch('handleRelatedPage', {pnum: (current + 1)}) : undefined}
            ...
            {#if pgcnt > 1 }
                <li class={cgroup === pgcnt ? 'list-item last disabled' : 'list-item last'}>
                    <button
                        ...
                        on:click={cgroup < pgcnt ? () =>dispatch('handleRelatedPage', {pnum:(cgroup * ppg + 1)}) : undefined}
...
.pagination-container {
 ...
 .pagination-container .pagination-list .list-item:not(.disabled) .page-link:hover .ico-pagination {
	 background-color: #515aa9;
	 color: #fff;
}
...
.list-item{   
 ...
 .pagination-container .pagination-list2 .list-item:not(.disabled) .page-link:hover .ico-pagination {
	 background-color: #515aa9;
	 color: #fff;
}
