...
<nav class="grid">
	<Menubutton bind:active bind:mainMenuVisible />
</nav>
	...
	<div class={['nav-items-container bg-custom-23 pb-10', (!active || mainMenuVisible) && 'hidden']}>
		<button
			class={[
				...
				activeMenuContent?.options &&
					activeMenuContent?.options[0]?.colTitle &&
		...
		{#if activeMenuContent.options}
			{#each activeMenuContent.options as option}
				{#if option?.colTitle}
					<div
						class="font-open-sans text-custom-1 text-base font-semibold mt-[1.0625rem] mb-[0.8125rem]"
					>
						{option.colTitle}
					</div>
				{/if}
