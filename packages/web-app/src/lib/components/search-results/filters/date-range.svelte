<script lang="ts">
  ...
  const startDateKey = $derived(`${dateId}-begin`);
  const endDateKey = $derived(`${dateId}-end`);
   ...
   */
  function customizeValidator(input: HTMLInputElement): { destroy: () => void } {
    /**
