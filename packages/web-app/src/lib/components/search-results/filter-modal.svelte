<script lang="ts">
  ...
  function buildFilterParams(filters: Filters): URLSearchParams {
    ...
    if (category) query.set(categoriesKey, category);
    else query.delete(categoriesKey);

    if (foundational) query.set(foundationalKey, String(foundational));
    else query.delete(foundationalKey);

    if (mappable) query.set(mappableKey, String(mappable));
    else query.delete(mappableKey);

    const orgString = getFilterStringFromObj(Object.entries(org));
    if (orgString) query.set(orgKey, orgString);
    else query.delete(orgKey);

    const typeFilterString = getFilterStringFromObj(Object.entries(typeFilter));
    if (typeFilterString) query.set(typeKey, typeFilterString);
    else query.delete(typeKey);

    const themeString = getFilterStringFromObj(Object.entries(theme));
    if (themeString) query.set(themeKey, themeString);
    else query.delete(themeKey);

    const eoCollectionString = getFilterStringFromObj(Object.entries(eoCollections));
    if (eoCollectionString) query.set(eoCollectionKey, eoCollectionString);
    else query.delete(eoCollectionKey);

    const sourceSystemString = getFilterStringFromObj(Object.entries(sourceSystem));
    if (sourceSystemString) query.set(sourceSystemKey, sourceSystemString);
    else query.delete(sourceSystemKey);
