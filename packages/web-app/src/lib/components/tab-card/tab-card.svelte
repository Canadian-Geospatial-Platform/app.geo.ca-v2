<script lang="ts">
  ...
  let activeTab = $derived(tabContentArray.find((tab: SelectOption) => tab.value === activeTabId) || undefined);
  let title = $derived(activeTab ? activeTab.label : '');

  // Initialize activeTabId when tabContentArray changes
  $effect(() => {
    if (tabContentArray.length > 0 && !activeTabId) {
      activeTabId = tabContentArray[0].value;
    }
  });
