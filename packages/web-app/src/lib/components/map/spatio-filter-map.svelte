...
<script lang="ts">
  ...
  const sConfig = $derived(JSON.stringify(config));
  ...
  function enforceRectangle(coords: number[][], movedIndex: number, newCoord: number[]): number[][] {
    ...
    const adjacentIndex1 = (movedIndex + 1) % 4;
    const adjacentIndex2 = (movedIndex + 3) % 4;
    ...
    try {
      ...
      cgpv.api.getMapViewer(mapId)?.onMapLayersLoaded(() => {
        ...
        const button = {
          ...
          children: cgpv.reactUtilities.react.createElement(
            'svg',
            {
              width: 24,
              height: 24,
            ...
            cgpv.reactUtilities.react.createElement('rect', { x: 3, y: 3, width: 18, height: 18 }),
            cgpv.reactUtilities.react.createElement('circle', { cx: 3, cy: 3, r: 2 }),
            cgpv.reactUtilities.react.createElement('circle', { cx: 3, cy: 21, r: 2 }),
            cgpv.reactUtilities.react.createElement('circle', { cx: 21, cy: 3, r: 2 }),
            cgpv.reactUtilities.react.createElement('circle', { cx: 21, cy: 21, r: 2 })
          ),
        ...
        translateInteraction.onTranslateEnded(() => {
          const lonLatCoords = layerGeometry.getFeatureCoords(bboxId, lonLatProjection)[0];
          bboxModify(lonLatCoords);
        });
         ...
         */
        modifyInteraction.onModifyStarted((sender: never, payload: { features: { item: (arg0: number) => Feature } }): void => {
        ...
        modifyInteraction.onModifyEnded((sender: never, payload: { features: { item: (arg0: number) => Feature } }) => {
        ...
        modifyInteraction.onModifyDragged((sender: never, payload: { features: { item: (arg0: number) => Feature } }) => {
