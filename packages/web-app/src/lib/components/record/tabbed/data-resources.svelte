<script lang="ts">
	...
	const downloadMetadataText = translations?.downloadMetadata
		? translations['downloadMetadata']
		: 'Download Metadata';
	const viewISO1915Text = translations?.viewISO1915
		? translations['viewISO1915']
		: 'View ISO1915 Record';
	...
	const resourcesNotAvailable = translations?.resourcesNotAvailable
		? translations['resourcesNotAvailable']
		: '';
...
{#if dataResourcesList.length > 0}
	...
	<div
		class="block md:hidden rounded bg-custom-1 px-5 drop-shadow-[0_0.1875rem_0.375rem_#00000029] divide-y divide-custom-17"
	>
